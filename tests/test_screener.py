...
BREAKOUT = {"require_above_sma": ["20", "50", "200"], "require_new_high": True,
...
def _feat(**overrides):
    base = {
        ...
        "rsi": 35.0, "avg_volume": 500_000, "last_volume": 1_000_000,
        "rel_volume": 2.0, "high": 10.0, "is_new_high": True,
...
def test_oversold_bounce_passes():
    r = evaluate_preset(OVERSOLD, _feat(price=8.0, rsi=28, rel_volume=2.5, pct_change_1d=3.0))
    assert r.passed and r.reasons == []


def test_oversold_bounce_rejects_not_oversold():
    r = evaluate_preset(OVERSOLD, _feat(rsi=60))
...
def test_oversold_bounce_rejects_falling_knife():
    ...
    r = evaluate_preset(OVERSOLD, _feat(rsi=25, rel_volume=3.0, pct_change_1d=-4.0))
...
def test_ma_bounce_passes_between_the_mas():
    ...
    r = evaluate_preset(MA_BOUNCE, _feat(price=10.0, sma20=9.5, sma50=10.5,
                                         avg_volume=500_000, rel_volume=1.2))
...
def test_breakout_passes_uptrend_new_high():
    r = evaluate_preset(BREAKOUT, _feat(price=30, sma20=28, sma50=25, sma200=20,
                                        is_new_high=True, avg_volume=200_000,
                                        roe_pct=25))
    assert r.passed, r.reasons
...
def test_breakout_rejects_not_new_high():
    r = evaluate_preset(BREAKOUT, _feat(price=30, sma20=28, sma50=25, sma200=20,
                                        is_new_high=False, avg_volume=200_000))
    assert any("NOT_NEW_HIGH" in x for x in r.reasons)
...
def test_features_computed_on_long_uptrend():
    ...
    assert feat["is_new_high"] is True
    assert feat["roe_pct"] == pytest.approx(25.0)


def test_features_relative_volume_excludes_current_bar():
    n = 60
    closes = list(np.linspace(20, 8, n - 1)) + [8.4]
    vols = [500_000] * (n - 1) + [1_500_000]
    feat = compute_screen_features(_ctx(closes, vols))
    # baseline = the preceding 20 bars (all 500k), NOT diluted by today's 1.5M →
    # exactly 3.0x, not 2.73x.
    assert feat["rel_volume"] == pytest.approx(3.0)
    assert feat["pct_change_1d"] > 0
    assert feat["rsi"] < 40                                   # long decline → oversold


def test_features_rsi_is_100_on_pure_uptrend():
    # A strictly rising series has no losses → RSI 100 (not None), so an RSI-min
    # criterion isn't spuriously failed.
    n = 60
    feat = compute_screen_features(_ctx(list(np.linspace(10, 30, n)), [500_000] * n))
    assert feat["rsi"] == pytest.approx(100.0)


def test_features_roe_derived_from_fundamentals_when_info_lacks_it():
    # Broker-configured env: ticker_info has no returnOnEquity, but the normalized
    # fundamentals do → ROE must still be computed (net_income / total_equity).
    n = 220
    ctx = _ctx(list(np.linspace(10, 30, n)), [300_000] * n, info={})
    ctx.fundamentals_raw = {"net_income": 20.0, "total_equity": 80.0, "total_debt": 40.0}
    feat = compute_screen_features(ctx)
    assert feat["roe_pct"] == pytest.approx(25.0)   # 20/80
    assert feat["debt_equity"] == pytest.approx(0.5)  # 40/80


def test_features_none_on_short_history():
    feat = compute_screen_features(_ctx([10, 11, 12], [1000, 1000, 1000]))
    assert feat["sma200"] is None and feat["rsi"] is None
