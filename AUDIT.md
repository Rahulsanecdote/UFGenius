...
| C2 safety unenforced | **Partially** resolved by restoring `executor.py`'s `RiskGuard`, which enforces max positions, single-position cap, cash reserve, per-trade risk, daily trade limit, bear-market guard, and duplicate-ticker. Still **not** enforced: `max_daily_loss_pct`/`max_weekly_loss_pct`, `cooldown_after_loss_hours`, `trade_earnings_week`, `stop_loss_required`, `paper_trade_days_required` — follow-up work. |
...
| Position cap counted only broker positions, not in-flight `pending_fill` entries | RiskGuard counts `max(broker positions, open tracker positions)`. |
...
Tests: **303 pass** (adds `test_live_execution_fixes.py`), 3 network tests
